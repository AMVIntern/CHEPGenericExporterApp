...
public class CombinedExcelReportService
{
    ...
    /// <summary>
    /// Explicit renames for columns that would collide after prefix stripping
    /// (e.g. both Top:BoardCount and Bot:BoardCount strip to "BoardCount").
    /// Keyed by original header (case-insensitive); value is the target attribute name.
    /// </summary>
    private static readonly Dictionary<string, string> GocatorColumnRenames = new(StringComparer.OrdinalIgnoreCase)
        {
            { "Top:BoardCount", "TBC" },
            { "Bot:BoardCount", "BBC" },
        };
    ...
    private static string StripGocatorColumnPrefix(string header)
    {
        ...
        if (GocatorColumnRenames.TryGetValue(header, out var renamed))
            return renamed;
