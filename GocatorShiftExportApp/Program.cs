...
class Program
{
    static void Main(string[] args)
    {
        ...
        string configPath = @"C:\AMV\GocatorShiftExporterApp\config.json"; // Adjust path as needed
        ...
        while (true)
        {
            ...
            var reportResult = excelGenerator.GenerateCombinedExcelReport();

            // Send email with the combined Excel file (and optional normalized CSV if row count exceeded limit)
            if (reportResult != null && !string.IsNullOrEmpty(reportResult.ExcelFilePath) && File.Exists(reportResult.ExcelFilePath))
            {
                // Extract shift and date from filename for email subject/body
                string fileName = Path.GetFileNameWithoutExtension(reportResult.ExcelFilePath);
                ...
                // Update email data for combined report (attach zip of normalized CSV when available)
                emailData.AttachmentPath = reportResult.ExcelFilePath;
                string normalizedAttachment = !string.IsNullOrEmpty(reportResult.NormalizedZipPath) && File.Exists(reportResult.NormalizedZipPath)
                    ? reportResult.NormalizedZipPath
                    : (!string.IsNullOrEmpty(reportResult.NormalizedCsvPath) && File.Exists(reportResult.NormalizedCsvPath) ? reportResult.NormalizedCsvPath : null);
                emailData.AdditionalAttachmentPaths = normalizedAttachment != null ? new List<string> { normalizedAttachment } : null;
                emailData.Subject = $"AMV Combined Report - Shift {shift} - {date}";
                emailData.Body = emailData.AdditionalAttachmentPaths != null
                    ? $"Please find attached the AMV Combined Report for Shift {shift}, {date}. The Excel file contains: GOCATOR, TOP_LEADING_STATION, TOP_TRAILING_STATION, SIDE_STATION, BOTTOM_LEADING_STATION. A zipped Combined Power BI report (CSV) is also included for use in Power BI."
                    : $"Please find attached the AMV Combined Report for Shift {shift}, {date}. The Excel file contains: GOCATOR, TOP_LEADING_STATION, TOP_TRAILING_STATION, SIDE_STATION, BOTTOM_LEADING_STATION.";
    ...
    private static bool SendEmail(EmailData emailData)
    {
        ...
        if (emailData.AdditionalAttachmentPaths != null)
        {
            foreach (var path in emailData.AdditionalAttachmentPaths)
            {
                if (!string.IsNullOrEmpty(path) && !File.Exists(path))
                {
                    Console.WriteLine($"Additional attachment not found: {path}");
                    return false;
                }
            }
        }
        ...
        const int smtpTimeoutMs = 300000; // 5 minutes
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                using (MailMessage mail = new MailMessage())
                {
                    ...
                    if (!string.IsNullOrEmpty(emailData.AttachmentPath) && File.Exists(emailData.AttachmentPath))
                        mail.Attachments.Add(new Attachment(emailData.AttachmentPath));
                    if (emailData.AdditionalAttachmentPaths != null)
                    {
                        foreach (var path in emailData.AdditionalAttachmentPaths)
                        {
            ...
            {
                Console.WriteLine($"SMTP attempt {attempt}/{maxAttempts} failed: {ex.Message}");
                Console.WriteLine($"Status Code: {ex.StatusCode}");
                if (ex.InnerException != null)
                    Console.WriteLine($"Inner: {ex.InnerException.Message}");
                if (attempt < maxAttempts)
                {
                    Console.WriteLine($"Retrying in {delayBetweenAttemptsMs / 1000} seconds...");
